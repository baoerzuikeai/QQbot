...
import (
	...
	"github.com/baoer/QQbot/internal/gemini"
...
func main() {
	...
	//读取响应消息
	fmt.Println("读取响应消息")
	gemini_client := gemini.InitClient()
	defer gemini_client.Close()
	send_msg_chan := make(chan string, 5)
	recive_msg_chan := make(chan string)
	go gemini.Chat(gemini_client, send_msg_chan, recive_msg_chan)
	for {
		...
		gm.Content = content
		go bot.SendGroupAtMessage(gm, send_msg_chan, recive_msg_chan)
	}
}
